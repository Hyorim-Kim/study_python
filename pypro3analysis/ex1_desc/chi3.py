# 이원카이제곱검정 - 두 개 이상의 집단(범주형 변인)을 대상으로 독립성, 동질성 검정
"""
독립성(관련성) 검정
...
print(data1['education'].unique())  # [1 2 3]  독립변수
print(data1['smoking'].unique())     # [1 2 3]  종속변수
# 교차표 작성
...
# 판정 : p:0.000818 < 0.05 이기 때문에 유의한 수준(α=0.05)에서 귀무가설을 기각한다.
# 발생된 데이터는 우연히 발생된 자료가 아니다.
# 교육수준과 흡연율 간에 관련이 있다.
# 참고 : Yeats의 연속성 보정 (자유도가 1일 때 보정 필요)
