...
# 두 개의 요인에 대한 집단(독립 변수) 각각이 종속변수의 평균에 영향을 주는지 검정
...
# reg = ols("머리둘레 ~ C(태아수) + C(관측자수), data=data).fit()  # 교호작용 확인X
# reg = ols("머리둘레 ~ C(태아수) + C(관측자수) + C(태아수):C(관측자수)", data=data).fit()  # 교호작용 확인O
...
# 데이터의 균형설계 확인
print(data2.groupby('poison').agg(len))  # poison 요인으로 구분된 집단 표본수는 16으로 동일
print(data2.groupby('treat').agg(len))      # treat 요인으로 구분된 집단 표본수는 12으로 동일
print(data2.groupby(['poison', 'treat']).agg(len))  # 요인별 레벨의 표본수는 4로 동일
# 모든 집단별 표본수가 동일하므로 균형설계가 잘 되었다고 할 수 있다.

result2 = ols('time ~ C(poison) * C(treat)', data=data2).fit()
print(anova_lm(result2))
#                                      df     sum_sq      mean_sq             F                  PR(>F)
# C(poison)                  2.0  1.033012  0.516506  23.221737  3.331440e-07  < 0.05이므로 귀무가설 기각
# C(treat)                      3.0  0.921206  0.307069  13.805582  3.777331e-06  < 0.05이므로 귀무가설 기각
# C(poison):C(treat)   6.0  0.250138  0.041690   1.874333    1.122506e-01  > 0.05이므로 상호작용효과는 발견할 수 없다.
