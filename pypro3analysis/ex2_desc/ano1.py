...
f 값 = 그룹 간 분산(Between Varaiance) / 그룹 내 분산(Within Variance)
...
import urllib.request  # read_csv 대신
...
data = data.dropna()  # na 버림
print(data.head(3), data.shape)  # (80, 4)

# plt.boxplot(data.score)
# plt.hist(data.score)
# plt.show()  # 이상치가 발견됨(outlier)

data = data.query('score <= 100')  # 조건 부여
print(data.shape)

# plt.hist(data.score)
# plt.show()  # 정규 분포 따름

# 분산 분석의 전제 조건 : 3가지를 충족할 때 의미있다.
# 독립성 : 각 집단은 서로 독립이어야 한다. (상관관계 확인)
# 정규성 : 각 집단은 정규분포를 따라야 한다. (shapiro, stats.ks_2samp - 정규성 검사)
...
score2 = m2['score']
score3 = m3['score']
print(np.mean(score1), np.mean(score2), np.mean(score3))  # 67.384   68.357   68.875

print('\n정규성 검정 : 만족하면 anova, 만족하지 않으면 kruskal-wallis test')
...
print('\n등분산성 검정 : 만족하면 anova, 만족하지 않으면 welch-anova')
print(stats.levene(score1, score2, score3).pvalue)    # 모수 검정     0.1132 > 0.05이므로 등분산성 만족
print(stats.bartlett(score1, score2, score3).pvalue)  # 비모수 검정(표준의 개수가 30개 이하일 때)
# 참고 : 등분산성을 만족하지 않은 경우 대처 방안
# 1 데이터를 normalization(정규화)으로 처리
# 2 standardization(표준화)로 처리
# 3 transformation 경우에 따라 자연 log를 붙임

# 교차표 : 교육방법별 건수, 빈도수 구하기
ctab1 = pd.crosstab(index=data['method'], columns='count')
ctab1.index = ['방법1', '방법3', '방법2']
print(ctab1)

# 교차표 : 교육방법별 만족여부 건수, 빈도수 구하기
ctab2 = pd.crosstab(data.method, data.survey)
ctab2.index = ['방법1', '방법3', '방법2']
ctab2.columns = ['만족', '불만족']
print(ctab2)

# anova_lm은 f 통계량을 위해 회귀분석 결과를 사용
import statsmodels.api as sm

reg = ols('score ~ method', data).fit()  # r 패턴
...
# 사후분석(post hoc) : ANOVA 검증 결과 유의미하다는 결론을 얻었을 때,
# 구체적으로 어떤 수준(들)에서 평균 차이가 나는지를 검증하는 방법
