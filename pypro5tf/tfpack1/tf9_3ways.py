...
# 공부시간에 따른 성적 결과 예측
# x_data = [[1.], [2.], [3.], [4.], [5.]]
x_data = np.array([1,2,3,4,5], dtype=np.float32)  # 상동
y_data = np.array([11,39,55,66,70], dtype=np.float32)
print(np.corrcoef(x_data, y_data))  # 0.95469145

print('\n1) Sequential api 사용 : 가장 단순한 방법. 레이어를 순서대로 쌓아 올린 완전 연결층 모델 생성')
model = Sequential()
model.add(Dense(units=2, input_dim=1, activation='relu'))  # hidden layer
model.add(Dense(units=1, activation='linear'))  # 마지막 출력층은 linear 고정
print(model.summary())

opti = optimizers.Adam(learning_rate=0.1)
model.compile(optimizer=opti, loss='mse', metrics=['mse'])
history = model.fit(x=x_data, y=y_data, batch_size=1, epochs=10, verbose=2)
...
print('\n2) functional api 사용 : 유연한 구조로 설계 가능.')
from keras.layers import Input
from keras.models import Model

inputs = Input(shape=(1,))  # 1개로 들어옴
# outputs = Dense(units=1, activation='linear')(inputs)  # 이전 층을 다음 층 함수의 입력으로 사용하도록 함
output1 = Dense(units=2, activation='relu')(inputs)  # 2개로 빠져나옴
...
history = model2.fit(x=x_data, y=y_data, batch_size=1, epochs=10, verbose=2)
...
print('\n3) sub classing 사용 : 동적인 구조로 설계 가능. 난이도 높은 네트워크 처리 가능')
x_data = np.array([[1.], [2.], [3.], [4.], [5.]], dtype=np.float32)  # matrix
y_data = np.array([11, 39, 55, 66, 70], dtype=np.float32)  # vector
...
history = model3.fit(x=x_data, y=y_data, batch_size=1, epochs=10, verbose=2)
...
history = model4.fit(x=x_data, y=y_data, batch_size=1, epochs=10, verbose=2)
