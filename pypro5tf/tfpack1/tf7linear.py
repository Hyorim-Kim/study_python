...
opti = SGD()  # 경사하강법 RMSprop, Adam으로 대체 가능
...
def trainModel(x, y):  # ***
    ...
    opti.apply_gradients(zip(grad, [w, b]))  # zip : 튜플의 형태로 차례로 접근할 수 있는 반복자(iterator)를 반환
    return loss
...
plt.plot(x, y_pred, 'b-', label='pred')  # 추세선
plt.xlabel('x')
plt.ylabel('y')
plt.legend()
plt.show()
