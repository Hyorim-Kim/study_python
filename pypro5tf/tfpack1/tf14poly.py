...
# 잔차 제곱의 평균을 구하는 함수
def compute_loss():
    y_pred = a * population_inc * population_inc + b * population_inc + c  # ax2 + bx + c
...
line_y = a * line_x * line_x + b * line_x + c  # 이차방정식 생성
...
print('\n------------------------')  # Sequential api를 사용한 방식
# Sequential 사용
model = Sequential([
    Dense(units=64, activation='relu', input_shape=(1,)),
    Dense(units=32, activation='relu'),
    Dense(units=1),  # 마지막 activation은 생략 가능
])
