...
model = Sequential()
model.add(Dense(units=32, activation='relu', input_shape=x_train.shape[1:]))  # 입력 8개, layer 안 node 32개
model.add(Dense(units=1))

model.compile(optimizer='adam', loss='mse', metrics=['mse'])
history = model.fit(x_train, y_train, epochs=50, validation_data=(x_valid, y_valid), verbose=2)  # 학습 도중 검증도 함
print('evaluate : ', model.evaluate(x_test, y_test, verbose=0))  # batch_size는 32가 기본값
x_new = x_test[:3]
y_pred = model.predict(x_new)
print('예측값 : ', y_pred.ravel())  # = flatten
print('실제값 : ', y_test[:3])

# 시각화
plt.plot(history.history['mse'], c='b', label='mse')  # mse와 loss는 비슷한 값이 출력됨
...
input_ = Input(shape=x_train.shape[1:])  # 8개
net1 = Dense(units=32, activation='relu')(input_)  # 은닉층을 2개로 나눔 (net1, net2)
net2 = Dense(units=32, activation='relu')(net1)
concat = Concatenate()([input_, net2])  # 입력층과 마지막 은닉층을 연결
output = Dense(units=1)(concat)

model2 = Model(inputs=[input_], outputs=[output])  # input, output은 여러개 될 수 있음

model2.compile(optimizer='adam', loss='mse', metrics=['mse'])
history = model2.fit(x_train, y_train, epochs=50, validation_data=(x_valid, y_valid), verbose=2)  # 학습 도중 검증도 함
print('evaluate : ', model2.evaluate(x_test, y_test, verbose=0))  # batch_size는 32가 기본값
x_new = x_test[:3]
...
input_a = Input(shape=[5], name='wide_input')  # 짧은 경로, 층이 복잡하면 name 작성하기
input_b = Input(shape=[6], name='deep_input')  # 깊은 경로
...
history = model3.fit((x_train_a, x_train_b), y_train, epochs=50, 
                                        validation_data=((x_valid_a, x_valid_b), y_valid), verbose=2)  # 학습 도중 검증도 함
print('evaluate : ', model3.evaluate((x_test_a, x_test_b), y_test, verbose=0))  # batch_size는 32가 기본값
