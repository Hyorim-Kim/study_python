...
# print(reuters.load_data(num_words=10000))  # 자주 등장하는 단어 10000개 사용
(train_data, train_label),(test_data, test_label) = reuters.load_data(num_words=10000)
print(train_data.shape, train_label.shape, test_data.shape, test_label.shape)  # (8982,) (8982,) (2246,) (2246,)
print(train_data[0])  # [1, 2, 2, 8, 43, 1, ..., 109, 15, 17, 12]
print(train_label[0])  # 3
print(set(train_label))  # {0, 1, 2, ...,  44, 45}

# 숫자로 매핑된 실제 데이터 보기 -------------------------------------
word_index = reuters.get_word_index()
# print(word_index.items())  # dict_items([('mdbl', 10996), ('fawc', 16260), ('degussa', 12089), ...
reverse_word_index = dict([value, key] for(key, value) in word_index.items())  # value, key 형태로 변환
# print(reverse_word_index)
# print(train_data[0])
decode_review = ' '.join([reverse_word_index.get(i) for i in train_data[0]])
# print(decode_review)  # [1, 2, 2, 8, 43, 10, 447, ...  : the of of mln loss for plc ...
# ---------------------------------------------------------------------------------
...
print(one_hot_test_labels[0])  # [0. 0. 0. 1. 0. 0. ...
...
# validation data : 선택적, validtion split을 사용한다면 필요없는 코드
x_val = x_train[:1000]
...
plt.plot(epochs, loss, 'r', label='val loss')
plt.xlabel('epochs')
plt.legend()
plt.show()
