...
# one-hot  :  keras:to_categorical, numpy:np.eys(), pandas:get_dummies(), sklearn:OneHotEncoder ...
...
# feature : 표준화 - 일반적으로 얘를 하면 성능이 좋아짐
...
def create_model_func(input_dim, output_dim, out_nodes, n, model_name='model'):
    ...
    def create_model():
        ...
        return model
    return create_model  # 내부함수의 주소를 리턴(클로저)
...
for model_name in history_dict:
    ...
    plt.plot(fpr, tpr, label='{}, acu value:{:.3f}'.format(model_name, auc(fpr, tpr)))
plt.xlabel('fpr')
plt.ylabel('tpr')
plt.legend()
plt.show()
