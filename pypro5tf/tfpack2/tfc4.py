...
# dataset 작성
xdata = np.random.random((1000, 12))  # 임의의 과목에 대한 시험점수라 가정
ydata = np.random.randint(5, size=(1000, 1))  # 시험과목은 5개  국어:0 ~ 체육:4라고 가정
print(xdata[:2])  # feature
print(ydata[:2])  # label
ydata = to_categorical(ydata, num_classes=5)  # one_hot encoding
print(ydata[:2])  # label
print([np.argmax(i) for i in ydata[:2]])  # argument 중 index 값이 가장 큰 것을 찾음(one_hot을 원복)

model = Sequential()
model.add(Dense(units=32, input_shape=(12, ), activation='relu'))
model.add(Dense(units=16, activation='relu'))
model.add(Dense(units=5, activation='softmax'))  # 카테고리가 5가지라서
print(model.summary())
...
print('분류 결과 : ', new_pred)  # 확률로 출력
print('분류 결과 합 : ', np.sum(new_pred))  # 1에 근사한 값 출력됨
