...
conn.execute(stmt, data1)  # 레코드 한 개 씩 저장
...
cursor = conn.execute("select * from mytab")  # 읽기
...
print(df1, type(df1))  # <class 'pandas.core.frame.DataFrame'>
print('--------------------')  # 위 방법으로 하지말기
df2 = pd.read_sql("select * from mytab", conn)  # 이 방법으로 바로 들어옴
print(df2)
