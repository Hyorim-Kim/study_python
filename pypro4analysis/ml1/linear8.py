...
print(train)
print(test)
# 비복원 추출 : random_state 지정
x_train = train[['studytime']]  # 2차원 matrix
# print(x_train, x_train.shape)
# (9, 1)  2차원(matrix)형태로 작성. 왜냐하면 sklearn의 분류 빛 예측 클래스는 feature:2차원, lable:1차원을 원함
...
model.fit(x_train, y_train)  # 모델 학습은 훈련용(train) 데이터로 함
y_pred = model.predict(x_test)  # 모델 검정은 검정용(test) 데이터로 함
...
print('결정계수 : ', r2)
print('결정계수 : ', r2_score(y_test, y_pred))  # 0.36298725192455683
# 결정계수는 분산을 기반으로 측정하므로 중심극한정리에 의해 표본 데이터가 많을수록 결정계수 수치도 높아진다.
# 무의미한 독립변수의 수가 늘면 결정계수 값이 늘어나는 경향이 있으므로 변수 선택에 주의 필요하다.
# 결정계수 값은 맹신 불가 ~
...
print()
# 새로운 hp로 mpg를 예측
new_hp = [[110]]  # 2차원으로 학습했으니 2차원에 넣어야함
